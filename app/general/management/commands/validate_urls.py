...
class Command(BaseCommand):
    ...
    def _check_url(self, url):
        """Return (success, reason, details) for a URL check."""
        parsed = urlparse(url)
        if parsed.scheme not in {"http", "https"}:
            return False, "Invalid scheme", ""

        head_ok, head_reason, head_details, head_status = self._request(url, method="HEAD")
        if head_ok:
            return True, "", ""

        if head_status in GET_FALLBACK_STATUS_CODES:
            get_ok, get_reason, get_details, _get_status = self._request(url, method="GET")
            if get_ok:
                return True, "", ""
    ...
    def _request(self, url, method):
        """Return (success, reason, details, status_code) for an HTTP request."""
        try:
